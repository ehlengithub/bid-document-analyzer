...
| 手机端客户版 | 客户版 HTML/PDF 默认按手机阅读优化，宽表在手机端折叠为带字段标签的卡片 |
...
客户版 PDF 默认导出为手机阅读版，页面宽度接近手机屏，文字保持 PDF 矢量可选中。需要 A4 打印版时再显式指定：
...
- 客户版不出现内部销售词。
- 客户版 HTML/PDF 优先适配手机端阅读，正文和表格字号不能过小；宽表在手机端折叠为带字段标签的卡片，不依赖横向拖动或 A4 缩小阅读。
